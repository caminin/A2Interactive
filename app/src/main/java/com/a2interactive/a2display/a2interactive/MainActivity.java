...
public class MainActivity extends Activity implements Detector.ImageListener, CameraDetector.CameraEventListener {
    ...
    long seconds_before_disappear = 2000;
    long seconds_before_reset = 15000;

    int duration_welcome_popup=3500;
    int duration_goodbye_popup=3500;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (permissionCheck == PackageManager.PERMISSION_GRANTED) {
            ...
            if(frontCam || rearCam){
                ...
                default_image = (VideoView) findViewById(R.id.default_image);
                default_image.setOnPreparedListener (new MediaPlayer.OnPreparedListener() {
                    @Override
                ...
                try {
                    ...
                    default_image.setVideoURI(Uri.parse("android.resource://" + getPackageName() + "/" + R.raw.basketconnection));
                } catch (Exception e) {
    ...
    public void google_vision() {
        ...
        mCameraSource = new CameraSource.Builder(context, google_detector)
                ...
                .setFacing(camera_google_source)
    ...
    private class A2FaceTrackerFactory implements MultiProcessor.Factory<com.google.android.gms.vision.face.Face> {
        ...
        public boolean man_detected;

        public A2FaceTrackerFactory() {
            this.man_detected =false;
            endingTask = new EndingTask();
            endingTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        }
    ...
    private class A2FaceTracker extends Tracker<com.google.android.gms.vision.face.Face> {
        ...
        public void onDone() {
            ...
            if(factory.man_detected==true){
                factory.man_detected=false;
                new MissingTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            }
        }
        ...
        public void waitingFinished(){
            ...
            if(factory.man_detected==false){
                new DetectionTask().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            }
        }
    ...
    private class DetectionTask extends AsyncTask<Void, String, Void> {
        ...
        @Override
        protected void onPostExecute(Void result) {
            Log.d("Tracker","Je détecte que la personne a disparu");
            resultFromDetection();
        }
    }

    private void resultFromDetection() {
        Log.d("Tracker","On a lancé la fin de session");
        if(ending_task==true){
            if(endingTask != null){
            ...
            mCameraSource.stop();
            initializeKenBurnsView();
            showImage(R.drawable.goodbye,duration_goodbye_popup);
            startDetector();
        }
        else{
            Log.d("Tracker","Un deuxieme thread cherche à terminer la session");
        }
    }

    private class EndingTask extends AsyncTask<Void, Boolean, Void> {
        private boolean is_already_ended=false;
        @Override
        protected Void doInBackground(Void... params) {
            long start=System.nanoTime();
            while((System.nanoTime()-start)<(seconds_before_reset*1000000));
            return null;
        }
